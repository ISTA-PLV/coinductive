...
def CoInd.le.bot [Inhabited (F PUnit)] : CoInd F :=
  cofix F (λ _ : PUnit => default) ⟨⟩

theorem CoInd.le.bot_eq [Inhabited (F PUnit)] :
...
def CoInd.le [Inhabited (F PUnit)] (c1 : CoInd F) (c2 : CoInd F) : Prop :=
  c1 = le.bot F ∨ coherent1 CoInd.le (QPF.unpack c1.unfold) (QPF.unpack c2.unfold)
...
theorem CoInd.le.rfl [Inhabited (F PUnit)] c:
  CoInd.le F c c := by
    ...
    subst_eqs
    right
    rcases (QPF.unpack (unfold F x1)) with ⟨_, _⟩ <;> try trivial
    constructor <;> try trivial
    simp

theorem CoInd.le.trans [Inhabited (F PUnit)] c1 c2 c3:
  CoInd.le F c1 c2 →
  CoInd.le F c2 c3 →
  CoInd.le F c1 c3 := by
    intro _ _
...
theorem CoInd.le.antisymm [Inhabited (F PUnit)] c1 c2:
  CoInd.le F c1 c2 →
  CoInd.le F c2 c1 →
  c1 = c2 := by
    intro h1 h2
...
instance [Inhabited (F PUnit)] : PartialOrder (CoInd F) where
  rel := CoInd.le F
  rel_refl := CoInd.le.rfl F _
  rel_trans := CoInd.le.trans F _ _ _
  rel_antisymm := CoInd.le.antisymm F _ _

noncomputable def CoInd.csup [Inhabited (F PUnit)] (c : CoInd F → Prop) : CoInd F :=
  cofix F (λ c =>
    if h : ∃ t, c t ∧ t ≠ CoInd.le.bot F then
      let .obj i _ := QPF.unpack (choose h).unfold;
      QPF.pack $ .obj i λ o c' =>
        ∃ c0, c c0 ∧ c0 ≠ CoInd.le.bot F ∧
...
theorem CoInd.csup_eq [Inhabited (F PUnit)] (c : CoInd F → Prop) :
  csup F c = if h : ∃ t, c t ∧ t ≠ CoInd.le.bot F then
      let .obj i _ := QPF.unpack (choose h).unfold
...
theorem CoInd.csup_eq_bot {c} [Inhabited (F PUnit)]:
  (¬∃ t, c t ∧ t ≠ le.bot F) →
  CoInd.csup F c = le.bot F := by
    intro h
    ext n
    induction n; rfl
    next n ih =>
    rw [CoInd.le.bot_eq, CoInd.csup_eq]
    simp [*, fold_approx]
...
theorem CoInd.csup_step {c} [Inhabited (F PUnit)] t i k:
  ...
  CoInd.csup F c = CoInd.fold F (QPF.pack $ .obj i
    λ o => CoInd.csup F λ c' => ∃ c0, c c0 ∧ c0 ≠ CoInd.le.bot F ∧
        let .obj i' k := QPF.unpack c0.unfold
...
theorem CoInd.csup_spec [Inhabited (F PUnit)] {c : CoInd F → Prop}
  ...
  · intro _
    ...
    subst_eqs
    by_cases h : ∃ t, c t ∧ t ≠ CoInd.le.bot F
    . right
      rcases h with ⟨t, _, _⟩
      ...
      simp [PartialOrder.rel, le, *] at heq
      rcases heq
      simp_all
      rename (_ ∧ _) => h
      cases h
